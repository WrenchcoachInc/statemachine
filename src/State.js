...
class State {
  ...
  fromJSON(j, context, flags) {
    ...
    for (const stateEventJson of j.stateEvents) {
      const stateEvent = ZeaEngine.sgFactory.constructClass(stateEventJson.type)
      stateEvent.fromJSON(stateEventJson, context)
      this.addStateEvent(stateEvent)
    }
    for (const activationActionJson of j.activationActions) {
      const activationAction = ZeaEngine.sgFactory.constructClass(
    ...
    for (const deactivationActionJson of j.deactivationActions) {
      const deactivationAction = ZeaEngine.sgFactory.constructClass(
...
ZeaEngine.sgFactory.registerClass('State', State)
